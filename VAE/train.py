...
# For using Bernoulli decoder
def loss_function(input_x, x, mu, sig):
    print(f"input_x:{input_x.shape}")
    print(f"just x:{x.shape}")
    print(f'viewed_x:{x.view(-1,784)}')
    BCE = F.binary_cross_entropy(input_x, x.view(-1, 784), reduction = 'sum')
    KLD = -0.5 * torch.sum(1 + sig - mu.pow(2) - sig.exp())
    return BCE, KLD

# For using gaussian decoder
def loss_function(input_x, x, mu, sig):
    MSE = F.mse_loss(input_x, x.view(-1,784),reduction='sum')
    KLD = -0.5 * torch.sum(1 + sig - mu.pow(2) - sig.exp())  # Both p and q are normal distribution
    return MSE, KLD
...
def train(train_loader):
    ...
    torch.save(model.state_dict(),f'C:/Users/heewo/Desktop/vscode_project/2022_DSAIL_Internship/VAE/checkpoints/VAE_model')
