...
class VAE(nn.Module):
    def __init__(self, input_dim, hidden_dim,num_layers,latent_dim):
        ...
        #output of encoder
        self.vae_mu = nn.Linear(half_dim, latent_dim)                         # (128, 20)
        self.vae_sig = nn.Linear(half_dim, latent_dim)                        # (128, 20)
        
        #Decoder part (vice versa of enocder part)
        self.vae_d = nn.ModuleList()
        self.vae_d.append(nn.Linear(latent_dim,half_dim))                     # (20, 128)
    ...
    def forward(self,x):
        ...
        '''
        If you want to use gaussian MLP decoder instead of bernoulli MlP decoder
        use sigmoid_output not just output below
        and use loss_function with MSE loss instead of binary cross entropy loss
        '''
